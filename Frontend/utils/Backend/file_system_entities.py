...
class file(file_system_entity):
    '''Says file but is covers folders as well, it is here the magic happens'''

    def compile_data(self, include_inside_method = True):
        '''Calculates all relevant information, started of as only counting the means so that's why the name is like that'''

        methods = self.count_method_lines(self.abs_path, include_inside_method) #Creates a structure of methods with relevant information
        
        self.mean_method_size, self.weight, self.max_lines, self.min_lines, self.mean_comments, self.mean_docstring, self.mean_comment_docstring = self.compile_method_data(methods) #collects all the data, yeah it is too much
    ...
    def find_parent(self, method, indent):
        '''
    ...
    def simple_inc(self, method, line, inside_docstring):
    ...
    def count_method_lines(self, file_path, include_inside_method = True):
        '''
        ...
        with open(file_path, 'r', errors="ignore") as file:
            for line in file: #goes through the file line by line
                ...
                if match != None and not inside_parameters and not inside_docstring: #There was a method!
                    ...
                    if self.find_end(line): #Checks if the parameters ends on the same line as the method
                        inside_parameters = False
                    else:
                    ...
                    if current_method is not None: #If we were currently counting a method, we add it to the methods dict
                        ...
                        if include_inside_method: #adds it's data to the parent as well if we count it as part of the parent
                            self.inc_parent(current_method)

                
                    parent = self.find_parent(current_method, indent_level) #find parent if there was one
                    ...
                    parent = self.find_parent(current_method, indent_level)
                    current_method = parent #Goes back one step, to either parent method or the "baseline"
                    if current_method:
                        self.simple_inc(current_method, line, inside_docstring) #increments line count if relevant

                elif current_method is not None and not inside_parameters: #normal line without method
                    self.simple_inc(current_method, line, inside_docstring)
                
                if inside_parameters: #checks if the parameter has ended
                    end_params_check = self.find_end(line)
                if end_params_check:
                    inside_parameters = False
    ...
    def compile_method_data(self, method_dict):
